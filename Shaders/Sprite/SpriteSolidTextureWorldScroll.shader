...
Shader "M8/Sprite/SolidTextureWorldScroll"
{
	Properties
	{
	...
	SubShader
	{
		...
		CGPROGRAM
			...
			v2fExt SpriteVert_Overlay(appdata_t IN)
			{
				...
				OUT.texcoord2 = mul(unity_ObjectToWorld, IN.vertex) * _Overlay_ST.xy + _Overlay_ST.zw + _OverlayParams.xy * _Time.y + half2(_OverlayParams.z * _CosTime.w, _OverlayParams.w * _SinTime.w);
			...
			{
				fixed4 c = tex2D(_Overlay, IN.texcoord2) * IN.color;
